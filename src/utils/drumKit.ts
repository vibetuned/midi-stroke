 ...
 * real pedal does.
 *
 * Pad numbers are General MIDI percussion: alternates are folded in (both
 * snare pads, all the cymbal pads, both pads per tom), so every pad on a GM
 * controller makes the sound it is labelled. This is the app's own numbering —
 * the score and playback use it. What a *controller* sends goes through the
 * editable pad map instead (utils/drumMap.ts), which starts from this table.
 */
...
// The pad tables live in drumPads.ts (no Tone there, so the node checks can
// load them); re-exported for the modules that already import them from here.
export { PAD_TO_VOICE, MEI_TO_PAD, padForScoreNote, type DrumVoiceKey } from './drumPads';

export interface DrumKit {
    /** Play a General MIDI pad. Returns false when the note maps to no voice. */
    trigger(pad: number, velocity?: number, time?: number): boolean;
    /** Play a voice by name — a controller hit, through the pad map
     *  (utils/drumMap.ts), which need not be General MIDI. */
    play(voice: DrumVoiceKey, velocity?: number, time?: number): void;
    ...
    /** Cut anything still ringing — the cymbals and the open hi-hat. */
    allOff(): void;
    dispose(): void;
}
...
export function createDrumKit(): DrumKit {
    ...
    const play = (key: DrumVoiceKey, velocity = 0.8, time?: number) => {
    ...
    return {
        ...
        allOff() {
            const now = Tone.now();
            // Only the long voices can still be sounding; the rest are over
            // in a tenth of a second whatever we do.
            Object.values(voices).forEach(v => v.choke?.(now));
        },
        dispose() {
