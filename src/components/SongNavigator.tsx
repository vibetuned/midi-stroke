...
export const SongNavigator: React.FC<SongNavigatorProps> = ({ onChangeRequest }) => {
    const { selectedSong, setSelectedSong, instrument } = useGame();
    ...
    if (!isLocal && files.length > 0) {
        ...
        const currentIdx = files.findIndex(f => `${f.path}/${f.name}` === normalized);
        if (currentIdx !== -1) {
            ...
            if (localIdx > 0) {
                ...
                prevTarget = `${t.path}/${t.name}`;
            }
            if (localIdx < collection.length - 1) {
                const t = collection[localIdx + 1];
                nextTarget = `${t.path}/${t.name}`;
            }
        }
    }
